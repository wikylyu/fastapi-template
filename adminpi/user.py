...
from api.response import success_response, failure_response,  PaginationResponse
...
@router.get('/users', response_model=PaginationResponse[schema.User])
async def find_users(query: str = '', status: UserStatus | None = None,
                     page: Annotated[int, Query(min=1)] = 1,
                     page_size: Annotated[int, Query(min=5, max=100)] = 10,
                     self: AdminStaff = Depends(get_current_staff),
